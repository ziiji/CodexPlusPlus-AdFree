...
- 同步上游 Codex++ v1.2.56。
- 支持未认证上游，并通过本地协议代理安全转发请求。
- 支持自定义 cc-switch 数据库路径，并修复 DreamSkin Windows 路径规范化。
- 修复历史会话、插件 marketplace、Skills 安装和供应商切换相关问题。
- 新增浮动面板基础模块与注入边界，改进 macOS DMG 引导式安装布局。
- 去除管理器赞助卡片、推荐页面、远程广告源和内置推广入口。
- 自动更新继续指向 `ziiji/CodexPlusPlus-AdFree`。
