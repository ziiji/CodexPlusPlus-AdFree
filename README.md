...
本仓库不是原项目的官方发布渠道。原项目名称、代码和版权归原作者及贡献者所有；本分支的修改按 MIT 许可证发布。
...
本分支版本号跟随上游版本，例如上游 `v1.2.34` 对应本分支 `v1.2.34`。每个上游版本只发布一个去广告构建。软件通过 Release 中的 `latest.json` 检查和下载安装包。
...
& "${env:ProgramFiles(x86)}\NSIS\makensis.exe" "/INPUTCHARSET" "UTF8" "/DVERSION=1.2.34" CodexPlusPlus.nsi
Pop-Location
```
