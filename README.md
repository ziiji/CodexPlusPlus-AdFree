...
本分支版本号跟随上游版本，例如上游 `v1.2.50` 对应本分支 `v1.2.50`。每个上游版本只发布一个去广告构建。软件通过 Release 中的 `latest.json` 检查和下载安装包。
...
& "${env:ProgramFiles(x86)}\NSIS\makensis.exe" "/INPUTCHARSET" "UTF8" "/DVERSION=1.2.50" CodexPlusPlus.nsi
Pop-Location
```
