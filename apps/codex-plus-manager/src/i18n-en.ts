...
export const EN_PLAIN: Record<string, string> = {
  ...
  "移除图片": "Remove image",
  "删掉图片只发文字,避免纯文本模型报错(模型看不到图)":
    "Remove images and send text only to avoid errors from text-only models",
  "视觉辅助分析": "Vision-assisted analysis",
  "图片先由视觉辅助模型(Qwen)转成文字描述,纯文本模型也能\"看图\"":
    "First convert the image to a text description with the vision assistant (Qwen), so text-only models can still \"see\" it",
  "当前使用中：保留此供应商的 auth 存档，避免 Codex 登录密钥覆盖供应商密钥":
    "Currently in use: keeps this provider's auth snapshot so the Codex login key does not overwrite the provider key",
