...
export const EN_PLAIN: Record<string, string> = {
  ...
  "会话文件": "Session file",
  "会话导入": "Session import",
  "请粘贴 Codex++ 分享链接。": "Paste a Codex++ session share link.",
  ...
  "搜索名称、作者、描述或标签": "Search name, author, description or tags",
  "脚本市场排版": "Script marketplace layout",
  "板块": "Sections",
  "列表": "List",
  "没有匹配的市场脚本。": "No marketplace scripts match.",
  "导入文件": "Import file",
  "会话分享链接": "Session share link",
  "粘贴 Codex++ 会话分享链接": "Paste Codex++ session share link",
  "导入链接": "Import link",
  "尝试从sub2api获取倍率显示": "Try fetching the multiplier from sub2api",
  "获取倍率": "Fetch multiplier",
  "保存前可先尝试从 /v1/sub2api/billing 获取上游倍率。": "Before saving, you can try fetching the upstream multiplier from /v1/sub2api/billing.",
  "非 Sub2API 供应商不会请求或显示倍率。": "Non-Sub2API providers won't request or display the multiplier.",
  "多模态模型直接接收图片,不经过任何处理": "Multimodal models receive images directly without processing",
  "移除图片": "Remove image",
  ...
  "图片先由视觉辅助模型(Qwen)转成文字描述,纯文本模型也能\"看图\"": "First convert the image to a text description with the vision assistant (Qwen), so text-only models can still \"see\" it",
  "当前使用中：保留此供应商的 auth 存档，避免 Codex 登录密钥覆盖供应商密钥": "Currently in use: keeps this provider's auth snapshot so the Codex login key does not overwrite the provider key",
  "安全提示：网页链接中的自定义 config.toml 和 auth.json 不会执行；管理工具只会使用上方字段生成受管配置。": "Security notice: Custom config.toml and auth.json in web links are never executed; the manager only uses the fields above to generate managed configuration.",
  "从 DreamSkin.cc 安装主题": "Install a theme from DreamSkin.cc",
  "检测到网页一键换肤请求。确认后会从固定社区 API 下载，并在本机重新校验大小、SHA-256、ZIP 清单与 Safe CSS。": "A one-click web theme request was detected. After confirmation, the package is downloaded from the fixed community API and its size, SHA-256, ZIP manifest and Safe CSS are verified locally.",
  "主题版本 ID": "Theme version ID",
  "链接不能携带任意下载地址、文件路径或命令；安装后主题会进入“我的主题”，不会自动重启 Codex。": "The link cannot carry arbitrary download URLs, file paths or commands. After installation, the theme is added to My themes and Codex is not restarted automatically.",
...
export const EN_TEMPLATE: Record<string, string> = {
  ...
  "来自 DreamSkin.cc 的已审核主题，共 {0} 套；安装前仍会在本机再次校验。": "{0} reviewed themes from DreamSkin.cc; each package is verified again locally before installation.",
  ...
  "清单更新时间：{0}，当前显示 {1} / {2}": "Manifest updated at: {0}; currently showing {1} / {2}",
